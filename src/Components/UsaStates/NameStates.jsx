...
const NameStates = () => {
  ...
  useEffect(() => {
    States.forEach((state) => {
      if (
        ...
        !correctAnswers.some((correctState) => correctState.toLowerCase() === state.toLowerCase())
      ) {
        setStateCount((count) => count + 1);
        setCorrectAnswers((answers) => {
          const newAnswers = [...answers];
          const newState = States.find((s) => s.toLowerCase() === state.toLowerCase());
  ...
  return (
    <div>
      <button className="t3button" onClick={handleClick}>Restart</button>
